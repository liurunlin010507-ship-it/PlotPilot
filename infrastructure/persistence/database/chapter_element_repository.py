...
class ChapterElementRepository:
    ...
    async def get_by_element(self, element_type: ElementType, element_id: str) -> List[ChapterElement]:
        """获取某个元素在哪些章节出现"""
        conn = self._get_connection()
        try:
            cursor = conn.cursor()
    ...
    async def get_by_chapter_and_type(self, chapter_id: str, element_type: ElementType) -> List[ChapterElement]:
        """获取章节中某类型的所有元素"""
        conn = self._get_connection()
        try:
            cursor = conn.cursor()
    ...
    async def exists(
        self, chapter_id: str, element_type: ElementType, element_id: str, relation_type: RelationType
    ) -> bool:
        """检查关联是否已存在"""
        conn = self._get_connection()
        try:
