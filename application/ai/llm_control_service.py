...
class LLMProfile(BaseModel):
    ...
    def _normalize_headers(cls, value: Dict[str, str]) -> Dict[str, str]:
        return {str(k).strip(): str(v).strip() for k, v in (value or {}).items() if str(k).strip() and str(v).strip()}
...
class LLMControlService:
    ...
    def _get_meta(self, key: str, default: str = "") -> str:
        row = self._db().fetch_one("SELECT value FROM llm_config_meta WHERE key = ?", (key,))
    ...
    def get_config(self) -> LLMControlConfig:
        ...
        try:
            rows = self._db().fetch_all("SELECT * FROM llm_profiles ORDER BY sort_order, created_at")
        ...
        for idx, profile in enumerate(sanitized.profiles):
            ...
            params_list.append(
                (
                    ...
                    row["extra_body"],
                    row["notes"],
                    row["use_legacy_chat_completions"],
                    row["sort_order"],
                )
            )
    ...
    def _sanitize_config(self, config: LLMControlConfig) -> LLMControlConfig:
        ...
        active_profile_id = (
            config.active_profile_id if config.active_profile_id in {p.id for p in profiles} else profiles[0].id
        )
    ...
    def _build_initial_config(self) -> LLMControlConfig:
        ...
        if anthropic_key and (llm_provider == "anthropic" or not llm_provider):
            ...
            profiles[2] = profiles[2].model_copy(
                update={
                    "api_key": gemini_key,
