...
class VolumeSummaryService:
    ...
    async def generate_act_summary(
        ...
        try:
            ...
            # 获取该幕涉及的伏笔
            foreshadowing_info = await self._get_foreshadowing_info(
                novel_id, act_node.chapter_start, act_node.chapter_end
            )
            ...
            # 调用 LLM
            response = await self.llm_service.generate(prompt, GenerationConfig(max_tokens=400, temperature=0.5))
    ...
    async def generate_volume_summary(
        ...
        try:
            ...
            volume_node = next(
                (n for n in all_nodes if n.node_type == NodeType.VOLUME and n.number == volume_number), None
            )
            ...
            response = await self.llm_service.generate(prompt, GenerationConfig(max_tokens=800, temperature=0.5))
    ...
    async def generate_part_summary(
        ...
        try:
            all_nodes = await self.story_node_repo.get_by_novel(novel_id)
            part_node = next((n for n in all_nodes if n.node_type == NodeType.PART and n.number == part_number), None)
            ...
            response = await self.llm_service.generate(prompt, GenerationConfig(max_tokens=500, temperature=0.5))
            ...
            return SummaryResult(
                success=True, summary=summary, tokens=len(summary) // 2, metadata={"part_id": part_node.id}
            )
    ...
    async def generate_checkpoint_summary(
        ...
        try:
            ...
            recent = sorted(
                [c for c in all_chapters if c.number <= current_chapter], key=lambda c: c.number, reverse=True
    ...
    def get_act_summary(self, novel_id: str, act_number: int) -> Optional[str]:
        ...
        try:
            all_nodes = self.story_node_repo.get_by_novel_sync(novel_id)
            act_node = next((n for n in all_nodes if n.node_type == NodeType.ACT and n.number == act_number), None)
    ...
    def _build_act_summary_prompt(
        ...
        """构建幕摘要 Prompt"""
        chapters_text = "\n".join(
            [f"第{ch['number']}章《{ch['title']}》: {ch['outline'][:100]}" for ch in chapter_info[:10]]
        )
...
请生成这一幕的摘要。"""
    ...
    def _build_volume_summary_prompt(
        ...
        acts_text = "\n\n".join([f"【第{act['number']}幕 {act['title']}】\n{act['summary']}" for act in act_summaries])
...
请生成这一卷的摘要。"""
    ...
    def _build_part_summary_prompt(
        ...
        """构建部摘要 Prompt"""
        volumes_text = "\n\n".join(
            [f"【第{vol['number']}卷 {vol['title']}】\n{vol['summary']}" for vol in volume_summaries]
        )
...
请生成这一部的摘要。"""
    ...
    def _build_checkpoint_summary_prompt(
        ...
        """构建检查点摘要 Prompt"""
        chapters_text = "\n".join(
            [f"第{ch['number']}章《{ch['title']}》: {ch['content_preview'][:200]}" for ch in chapter_info]
        )
            ...
            for f in registry.foreshadowings:
                if chapter_start and chapter_end:
                    ...
                    if (
                        ...
                        and f.resolved_in_chapter >= chapter_start
                        and f.resolved_in_chapter <= chapter_end
                    ):
    ...
    async def _generate_volume_summary_from_chapters(
        ...
        try:
            ...
            volume_chapters = [
                ...
                if volume_node.chapter_start
                and volume_node.chapter_end
