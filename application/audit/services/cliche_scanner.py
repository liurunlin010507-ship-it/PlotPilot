...
class ClicheScanner:
    ...
    def __init__(self):
        # 预编译所有正则表达式以提高性能
        self.compiled_patterns = [(re.compile(pattern), name) for pattern, name in AI_CLICHE_PATTERNS]
    ...
    def scan_cliches(self, text: str) -> List[ClicheHit]:
        ...
        for compiled_pattern, pattern_name in self.compiled_patterns:
            for match in compiled_pattern.finditer(text):
                hit = ClicheHit(
                    pattern=pattern_name, text=match.group(0), start=match.start(), end=match.end(), severity="warning"
                )
                hits.append(hit)
