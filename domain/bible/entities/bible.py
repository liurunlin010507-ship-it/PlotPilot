...
class Bible(BaseEntity):
    ...
    def add_character(self, character: Character) -> None:
        ...
        if any(c.character_id == character.character_id for c in self._characters):
            raise InvalidOperationError(f"Character with id '{character.character_id.value}' already exists")
    ...
    def remove_character(self, character_id: CharacterId) -> None:
        ...
        if character is None:
            raise InvalidOperationError(f"Character with id '{character_id.value}' not found")
    ...
    def add_world_setting(self, setting: WorldSetting) -> None:
        ...
        if any(s.id == setting.id for s in self._world_settings):
            raise InvalidOperationError(f"World setting with id '{setting.id}' already exists")
    ...
    def remove_world_setting(self, setting_id: str) -> None:
        ...
        if setting is None:
            raise InvalidOperationError(f"World setting with id '{setting_id}' not found")
        self._world_settings.remove(setting)

    def add_location(self, location: Location) -> None:
        """添加地点"""
        if any(loc.id == location.id for loc in self._locations):
            raise InvalidOperationError(f"Location with id '{location.id}' already exists")
    ...
    def remove_location(self, location_id: str) -> None:
        ...
        if location is None:
            raise InvalidOperationError(f"Location with id '{location_id}' not found")
        self._locations.remove(location)

    def add_timeline_note(self, note: TimelineNote) -> None:
        """添加时间线笔记"""
        if any(n.id == note.id for n in self._timeline_notes):
            raise InvalidOperationError(f"TimelineNote with id '{note.id}' already exists")
    ...
    def remove_timeline_note(self, note_id: str) -> None:
        ...
        if note is None:
            raise InvalidOperationError(f"TimelineNote with id '{note_id}' not found")
    ...
    def add_style_note(self, note: StyleNote) -> None:
        ...
        if any(n.id == note.id for n in self._style_notes):
            raise InvalidOperationError(f"StyleNote with id '{note.id}' already exists")
    ...
    def remove_style_note(self, note_id: str) -> None:
        ...
        if note is None:
            raise InvalidOperationError(f"StyleNote with id '{note_id}' not found")
