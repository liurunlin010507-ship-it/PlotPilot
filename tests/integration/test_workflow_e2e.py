...
def mock_dependencies():
    ...
    consistency_checker.check_all.return_value = ConsistencyReport(issues=[], warnings=[], suggestions=[])
...
class TestCompleteGenerationFlow:
    ...
    async def test_outline_to_content_flow(self, workflow, mock_dependencies):
        ...
        # 执行生成
        result = await workflow.generate_chapter(novel_id=novel_id, chapter_number=chapter_number, outline=outline)
    ...
    async def test_multiple_chapters_generation(self, workflow):
        ...
        outlines = ["Chapter 1: Introduction", "Chapter 2: Rising action", "Chapter 3: Climax"]

        results = []
        for i, outline in enumerate(outlines, 1):
            result = await workflow.generate_chapter(novel_id=novel_id, chapter_number=i, outline=outline)
    ...
    async def test_large_content_handling(self, workflow, mock_dependencies):
        ...
        mock_dependencies["llm_service"].generate = AsyncMock(
            return_value=LLMResult(content=large_content, token_usage=TokenUsage(input_tokens=1000, output_tokens=3000))
        )

        result = await workflow.generate_chapter(
            novel_id="test-novel-3", chapter_number=1, outline="Long chapter outline"
        )
...
class TestPerformance:
    ...
    async def test_100_chapter_generation_simulation(self, workflow):
        ...
        for i in range(1, 11):
            result = await workflow.generate_chapter(
                novel_id=novel_id, chapter_number=i, outline=f"Chapter {i} outline"
            )
            assert isinstance(result, GenerationResult)
...
class TestErrorHandling:
    ...
    async def test_llm_failure_handling(self, workflow, mock_dependencies):
        """测试 LLM 失败时的处理"""
        # 模拟 LLM 失败
        mock_dependencies["llm_service"].generate = AsyncMock(side_effect=RuntimeError("LLM service unavailable"))

        with pytest.raises(RuntimeError, match="LLM service unavailable"):
            await workflow.generate_chapter(novel_id="test-novel", chapter_number=1, outline="Test outline")
    ...
    async def test_invalid_input_handling(self, workflow):
        ...
        with pytest.raises(ValueError, match="chapter_number must be positive"):
            await workflow.generate_chapter(novel_id="test-novel", chapter_number=0, outline="Test outline")

        # 测试空大纲
        with pytest.raises(ValueError, match="outline cannot be empty"):
            await workflow.generate_chapter(novel_id="test-novel", chapter_number=1, outline="")
...
class TestIntegrationWithComponents:
    ...
    async def test_context_builder_integration(self, workflow, mock_dependencies):
        ...
        await workflow.generate_chapter(novel_id="test-novel", chapter_number=5, outline="Chapter 5 outline")

        # 验证 ContextBuilder 被正确调用
        mock_dependencies["context_builder"].build_context.assert_called_once_with(
            novel_id="test-novel", chapter_number=5, outline="Chapter 5 outline", max_tokens=35000
        )

    @pytest.mark.asyncio
    async def test_consistency_checker_integration(self, workflow, mock_dependencies):
        """测试与 ConsistencyChecker 的集成"""
        result = await workflow.generate_chapter(novel_id="test-novel", chapter_number=1, outline="Test outline")
