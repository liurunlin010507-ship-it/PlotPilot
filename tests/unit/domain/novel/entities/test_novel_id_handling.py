...
def test_novel_equality_uses_base_entity_id():
    ...
    novel1 = Novel(id=NovelId("novel-1"), title="测试小说1", author="作者1", target_chapters=10)
    novel2 = Novel(id=NovelId("novel-1"), title="测试小说2", author="作者2", target_chapters=20)
    # 相同 ID 应该相等
...
def test_novel_can_be_used_in_set():
    ...
    novel1 = Novel(id=NovelId("novel-1"), title="测试小说1", author="作者1", target_chapters=10)
    novel2 = Novel(id=NovelId("novel-1"), title="测试小说2", author="作者2", target_chapters=20)
    novel3 = Novel(id=NovelId("novel-2"), title="测试小说3", author="作者3", target_chapters=30)
...
def test_novel_can_be_used_as_dict_key():
    ...
    novel1 = Novel(id=NovelId("novel-1"), title="测试小说", author="测试作者", target_chapters=10)

    novel_dict = {novel1: "value1"}
    assert novel_dict[novel1] == "value1"

    # 相同 ID 的 Novel 应该能访问相同的值
    novel2 = Novel(id=NovelId("novel-1"), title="另一个标题", author="另一个作者", target_chapters=20)
