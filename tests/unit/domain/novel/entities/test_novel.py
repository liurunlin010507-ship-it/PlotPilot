...
def test_novel_creation():
    """测试创建小说"""
    novel = Novel(id=NovelId("novel-1"), title="测试小说", author="测试作者", target_chapters=10)
...
def test_novel_add_chapter():
    """测试添加章节"""
    novel = Novel(id=NovelId("novel-1"), title="测试小说", author="测试作者", target_chapters=10)
    chapter = Chapter(id="chapter-1", novel_id=NovelId("novel-1"), number=1, title="第一章")
...
def test_novel_add_chapter_non_sequential_raises_error():
    """测试添加非连续章节抛出异常"""
    novel = Novel(id=NovelId("novel-1"), title="测试小说", author="测试作者", target_chapters=10)
    chapter1 = Chapter(id="chapter-1", novel_id=NovelId("novel-1"), number=1, title="第一章")
    chapter3 = Chapter(id="chapter-3", novel_id=NovelId("novel-1"), number=3, title="第三章")
...
def test_novel_completed_chapters():
    ...
    # 添加不同状态的章节
    chapter1 = Chapter(
        id="chapter-1", novel_id=NovelId("novel-1"), number=1, title="第一章", status=ChapterStatus.COMPLETED
    )
    chapter2 = Chapter(
        id="chapter-2", novel_id=NovelId("novel-1"), number=2, title="第二章", status=ChapterStatus.DRAFT
    )
    chapter3 = Chapter(
        id="chapter-3", novel_id=NovelId("novel-1"), number=3, title="第三章", status=ChapterStatus.COMPLETED
    )
