...
class TestConsistencyChecker:
    ...
    def test_check_character_consistency_no_issues(self):
        ...
        character = Character(id=char_id, name="张三", description="勇敢的战士")
        self.bible.add_character(character)

        issues = self.checker.check_character_consistency(
            character_id="char-1", action="勇敢地冲向敌人", context=self.context
        )
    ...
    def test_check_character_consistency_character_not_found(self):
        """测试角色一致性检查 - 角色不存在"""
        issues = self.checker.check_character_consistency(
            character_id="nonexistent", action="做了某事", context=self.context
        )
    ...
    def test_check_relationship_consistency_no_issues(self):
        ...
        # 添加现有关系
        relation = Relationship(relation_type=RelationType.FRIEND, established_in_chapter=1, description="好朋友")
        self.relationship_graph.add_relationship(char1_id, char2_id, relation)

        issues = self.checker.check_relationship_consistency(
            char1="char-1", char2="char-2", new_relation="friend", context=self.context
        )
    ...
    def test_check_relationship_consistency_character_not_found(self):
        """测试关系一致性检查 - 角色不存在"""
        issues = self.checker.check_relationship_consistency(
            char1="nonexistent", char2="char-2", new_relation="friend", context=self.context
        )
    ...
    def test_check_event_logic_no_issues(self):
        ...
        event = {"type": "conflict", "description": "主角与敌人战斗", "involved_characters": ["char-1"], "chapter": 5}

        issues = self.checker.check_event_logic(event=event, context=self.context)

        assert len(issues) == 0

    def test_check_event_logic_character_not_found(self):
        """测试事件逻辑检查 - 涉及的角色不存在"""
        event = {"type": "conflict", "description": "战斗", "involved_characters": ["nonexistent"], "chapter": 5}
    ...
    def test_check_foreshadowing_no_issues(self):
        ...
        issues = self.checker.check_foreshadowing(foreshadowing_id="f-1", context=self.context)

        assert len(issues) == 0

    def test_check_foreshadowing_not_found(self):
        """测试伏笔检查 - 伏笔不存在"""
        issues = self.checker.check_foreshadowing(foreshadowing_id="nonexistent", context=self.context)
    ...
    def test_check_all_empty_state(self):
        ...
        report = self.checker.check_all(chapter_state=chapter_state, context=self.context)
    ...
    def test_check_all_with_character_actions(self):
        ...
        chapter_state = ChapterState(
            new_characters=[],
            character_actions=[{"character_id": "char-1", "action": "做出决定", "chapter": 5}],
            relationship_changes=[],
            foreshadowing_planted=[],
            foreshadowing_resolved=[],
            events=[],
    ...
    def test_check_all_with_issues(self):
        ...
        chapter_state = ChapterState(
            new_characters=[],
            character_actions=[{"character_id": "nonexistent", "action": "做出决定", "chapter": 5}],
            relationship_changes=[],
            foreshadowing_planted=[],
            foreshadowing_resolved=[],
            events=[],
