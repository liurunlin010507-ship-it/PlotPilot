...
def test_plot_point_creation():
    ...
    plot_point = PlotPoint(
        chapter_number=1, point_type=PlotPointType.OPENING, description="故事开端", tension=TensionLevel.LOW
    )
...
def test_plot_point_chapter_number_validation():
    ...
    with pytest.raises(ValueError, match="Chapter number must be >= 1"):
        PlotPoint(chapter_number=0, point_type=PlotPointType.OPENING, description="故事开端", tension=TensionLevel.LOW)

    with pytest.raises(ValueError, match="Chapter number must be >= 1"):
        PlotPoint(chapter_number=-1, point_type=PlotPointType.OPENING, description="故事开端", tension=TensionLevel.LOW)


def test_plot_point_description_validation():
    """测试 PlotPoint 描述验证"""
    with pytest.raises(ValueError, match="Description cannot be empty"):
        PlotPoint(chapter_number=1, point_type=PlotPointType.OPENING, description="", tension=TensionLevel.LOW)
...
def test_plot_point_equality():
    ...
    plot_point3 = PlotPoint(
        chapter_number=2, point_type=PlotPointType.RISING_ACTION, description="冲突升级", tension=TensionLevel.MEDIUM
    )
...
def test_plot_point_with_different_types():
    ...
    climax = PlotPoint(
        chapter_number=10, point_type=PlotPointType.CLIMAX, description="故事高潮", tension=TensionLevel.PEAK
    )

    resolution = PlotPoint(
        chapter_number=15, point_type=PlotPointType.RESOLUTION, description="故事结局", tension=TensionLevel.LOW
    )
