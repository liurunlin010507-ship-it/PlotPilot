...
class TestChapterIndexingService:
    ...
    def service(self, mock_vector_store, mock_embedding_service):
        """创建服务实例"""
        return ChapterIndexingService(vector_store=mock_vector_store, embedding_service=mock_embedding_service)

    @pytest.mark.asyncio
    async def test_index_chapter_summary_calls_embedding(self, service, mock_embedding_service):
        """测试索引章节摘要时调用 embedding 服务"""
        summary = "这是第一章的摘要"

        await service.index_chapter_summary(novel_id="novel-123", chapter_number=1, summary=summary)
    ...
    async def test_index_chapter_summary_writes_to_vector_store(
        ...
        await service.index_chapter_summary(novel_id=novel_id, chapter_number=chapter_number, summary=summary)
    ...
    @pytest.mark.asyncio
    async def test_ensure_collection_creates_if_not_exists(self, service, mock_vector_store):
        ...
        mock_vector_store.create_collection.assert_called_once_with(collection="novel_novel-123_chunks", dimension=1536)

    @pytest.mark.asyncio
    async def test_ensure_collection_skips_if_exists(self, service, mock_vector_store):
        """测试 ensure_collection 在 collection 已存在时跳过创建"""
        novel_id = "novel-123"
        mock_vector_store.list_collections.return_value = ["novel_novel-123_chunks"]
    ...
    async def test_index_chapter_summary_validates_novel_id(self, service):
        ...
        with pytest.raises(ValueError, match="novel_id cannot be empty"):
            await service.index_chapter_summary(novel_id="", chapter_number=1, summary="摘要")
    ...
    async def test_index_chapter_summary_validates_chapter_number(self, service):
        ...
        with pytest.raises(ValueError, match="chapter_number must be >= 1"):
            await service.index_chapter_summary(novel_id="novel-123", chapter_number=0, summary="摘要")
    ...
    async def test_index_chapter_summary_validates_summary(self, service):
        ...
        with pytest.raises(ValueError, match="summary cannot be empty"):
            await service.index_chapter_summary(novel_id="novel-123", chapter_number=1, summary="")

        with pytest.raises(ValueError, match="summary cannot be empty"):
            await service.index_chapter_summary(novel_id="novel-123", chapter_number=1, summary="   ")

    @pytest.mark.asyncio
    async def test_index_bible_snippet_writes_to_vector_store(self, service, mock_vector_store):
        """测试索引 Bible 片段时写入向量存储"""
        novel_id = "novel-123"
        chapter_number = 5
        snippet = "这是一段 Bible 片段"

        await service.index_bible_snippet(novel_id=novel_id, chapter_number=chapter_number, snippet=snippet)
    ...
    @pytest.mark.asyncio
    async def test_index_bible_snippet_with_snippet_id(self, service, mock_vector_store):
        ...
        await service.index_bible_snippet(
            novel_id=novel_id, chapter_number=chapter_number, snippet=snippet, snippet_id=snippet_id
        )
    ...
    async def test_index_bible_snippet_validates_parameters(self, service):
        ...
        # 验证 novel_id
        with pytest.raises(ValueError, match="novel_id cannot be empty"):
            await service.index_bible_snippet(novel_id="", chapter_number=1, snippet="片段")

        # 验证 chapter_number
        with pytest.raises(ValueError, match="chapter_number must be >= 1"):
            await service.index_bible_snippet(novel_id="novel-123", chapter_number=0, snippet="片段")

        # 验证 snippet
        with pytest.raises(ValueError, match="snippet cannot be empty"):
            await service.index_bible_snippet(novel_id="novel-123", chapter_number=1, snippet="")
    ...
    @pytest.mark.asyncio
    async def test_collection_isolation_between_novels(self, service, mock_vector_store):
        """测试不同小说使用不同的 collection"""
        # 索引第一部小说
        await service.index_chapter_summary(novel_id="novel-1", chapter_number=1, summary="小说1的摘要")

        # 索引第二部小说
        await service.index_chapter_summary(novel_id="novel-2", chapter_number=1, summary="小说2的摘要")
