...
class TestGetVectorStore:
    ...
    def test_get_vector_store_returns_qdrant_when_legacy_env_set(self):
        ...
        with patch.dict(
            os.environ, {"QDRANT_ENABLED": "true", "QDRANT_HOST": "localhost", "QDRANT_PORT": "6333"}, clear=True
        ):
            ...
            with patch("infrastructure.ai.qdrant_vector_store.QdrantVectorStore") as mock_qdrant:
                ...
                assert result is mock_instance
                # 验证使用正确的参数初始化
                mock_qdrant.assert_called_once_with(host="localhost", port=6333, api_key=None)

    def test_get_vector_store_returns_qdrant_when_store_type_set(self):
        """显式设置 VECTOR_STORE_TYPE=qdrant 时返回 QdrantVectorStore 实例。"""
        with patch.dict(
            os.environ, {"VECTOR_STORE_TYPE": "qdrant", "QDRANT_HOST": "localhost", "QDRANT_PORT": "6333"}, clear=True
        ), patch("infrastructure.ai.qdrant_vector_store.QdrantVectorStore") as mock_qdrant:
            mock_instance = MagicMock()
            mock_qdrant.return_value = mock_instance
    ...
    def test_get_vector_store_with_custom_host_port(self):
        ...
        with patch.dict(
            os.environ,
            {"VECTOR_STORE_TYPE": "qdrant", "QDRANT_HOST": "qdrant.example.com", "QDRANT_PORT": "6334"},
            clear=True,
        ), patch("infrastructure.ai.qdrant_vector_store.QdrantVectorStore") as mock_qdrant:
            mock_instance = MagicMock()
            mock_qdrant.return_value = mock_instance
            ...
            mock_qdrant.assert_called_once_with(host="qdrant.example.com", port=6334, api_key=None)
    ...
    def test_get_vector_store_with_api_key(self):
        ...
        with patch.dict(
            ...
            mock_qdrant.assert_called_once_with(host="localhost", port=6333, api_key="test-api-key")

    def test_get_vector_store_uses_qdrant_default_values(self):
        """只设置 qdrant 类型时使用默认 host/port。"""
        with patch.dict(os.environ, {"VECTOR_STORE_TYPE": "qdrant"}, clear=True):
            with patch("infrastructure.ai.qdrant_vector_store.QdrantVectorStore") as mock_qdrant:
                mock_instance = MagicMock()
                mock_qdrant.return_value = mock_instance
