...
def get_service(planning_service: ContinuousPlanningService = Depends(get_planning_service)) -> StoryStructureService:
    """获取故事结构服务实例
    ...
    return StoryStructureService(repository, chapter_repository=chapter_repo, planning_service=planning_service)
...
async def get_structure_tree(novel_id: str, service: StoryStructureService = Depends(get_service)):
    """获取小说的完整结构树"""
    try:
...
async def get_children(
    novel_id: str, parent_id: Optional[str] = None, service: StoryStructureService = Depends(get_service)
):
    """获取子节点（用于渐进式加载）"""
    try:
        return {"parent_id": parent_id, "children": await service.get_children(novel_id, parent_id)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/novels/{novel_id}/structure/nodes")
async def create_node(novel_id: str, request: CreateNodeRequest, service: StoryStructureService = Depends(get_service)):
    """创建节点"""
    try:
...
async def update_node(
    novel_id: str, node_id: str, request: UpdateNodeRequest, service: StoryStructureService = Depends(get_service)
):
    """更新节点"""
    try:
        node = await service.update_node(
            node_id=node_id, title=request.title, description=request.description, number=request.number
        )
        return {"success": True, "node": node}
    except ValueError as e:
...
async def delete_node(novel_id: str, node_id: str, service: StoryStructureService = Depends(get_service)):
    """删除节点"""
    try:
...
async def reorder_nodes(novel_id: str, request: ReorderRequest, service: StoryStructureService = Depends(get_service)):
    """重新排序节点"""
    try:
...
async def update_chapter_ranges(novel_id: str, service: StoryStructureService = Depends(get_service)):
    """更新章节范围"""
    try:
...
async def create_default_structure(
    novel_id: str, request: CreateDefaultStructureRequest, service: StoryStructureService = Depends(get_service)
):
    """创建默认结构（AI 动态规划）
    ...
    try:
        result = await service.create_default_structure(
            novel_id=novel_id, total_chapters=request.total_chapters, structure_preference=request.structure_preference
        )
